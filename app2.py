...
def download_video(url, channel_name):
    ...
    thumbnail_stream = yt.streams.get_by_itag(22)
    if thumbnail_stream:
        thumbnail_file_name = thumbnail_stream.download(filename=yt.thumbnail_url.split("/")[-1])
        ...
        upload_metadata_to_api(video_metadata)

        return video_metadata
    else:
        print("Thumbnail download failed. No thumbnail stream found.")
def download_playlist(playlist_url, channel_name):
    ...
    # Integrate metadata upload to API here
    for video_metadata in metadata["Playlist"]["Videos"]:
        upload_metadata_to_api(video_metadata)

def upload_metadata_to_api(video_metadata):
    print("Uploading metadata to the API... #######################3")
    title = video_metadata["Title"]
    author = video_metadata["Author"]
    media_url = video_metadata["MediaURL"]
    thumbnail_url = video_metadata["thumbnails"][0]["url"]
    ...
    url = "http://onek.dev.eklavya.me/api/contents"
    token = st.secrets['STRAPI_TOKEN']  # Replace with your API token
    ...
    print("Data uploaded to the API successfully")
    print("Response Body:", api_response.json())
